...
function Skeleton({ className }: { className?: string }) {
    return (
        <div
            className={cn('animate-pulse rounded bg-muted/25', className)}
            aria-hidden
        />
    );
}
...
function BurndownChart({
    ...
    if (points.length < 2) {
        return (
            <p className="text-sm text-muted">
                No burndown data for this project yet.
            </p>
        );
    }
    ...
    const y = (v: number) =>
        plotBottom - ((v - min) / (max - min || 1)) * (plotBottom - padTop);
...
export function ProjectProgressDashboard({ authed }: { authed: boolean }) {
    ...
    useEffect(() => {
        ...
        void (async () => {
            ...
            startTransition(async () => {
                setError(null);
                const res = await fetch('/api/bitmap/clients');
                if (cancelled) return;
                if (!res.ok) {
                ...
                if (cancelled) return;
                const list = [
                    ...((data.clients as BitmapClientOption[]) ?? []),
                ];
    ...
    useEffect(() => {
        ...
        void (async () => {
            ...
            const cached = getCachedProjects(clientId, projectStatus);
            if (cached) {
            ...
            startTransition(async () => {
                ...
                setCachedProjects(clientId, list, projectStatus);
                applyProjects(list);
            });
        })();
    ...
    useEffect(() => {
        ...
        void (async () => {
            ...
            const cached = getCachedDashboard(projectId);
            if (cached) {
                setDashboard(cached);
                setDashboardLoading(false);
                return;
            ...
            try {
                ...
                if (cancelled || requestId !== dashboardRequestId.current) {
                    return;
                }
                if (!res.ok) {
                    const data = await res.json().catch(() => ({}));
                ...
                setCachedDashboard(projectId, next);
                setDashboard(next);
            } catch (err) {
                if (cancelled || requestId !== dashboardRequestId.current) {
                    return;
        ...
        <div className="space-y-6">
          ...
          live Jira Cloud API v3 when configured.
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
                    ...
                    <label className="flex w-full flex-col gap-1 text-sm sm:w-36">
                        ...
                        <Select
                            ...
                            onChange={(e) =>
                                setProjectStatus(
                                    e.target.value as ProjectListStatus,
                                )
                            }
                    ...
                    <label className="flex flex-[1.2] flex-col gap-1 text-sm">
                        ...
                        <Select
                            ...
                            disabled={
                                pending || !clientId || projects.length === 0
                            }
                        >
                            {projects.length === 0 ? (
                                <option value="">
                    ...
                    <div className="flex gap-2">
                        ...
                        <RefreshButton
                            ...
                            disabled={!projectId}
                            onClick={() =>
                                loadDashboard(projectId, { refresh: true })
                            }
            ...
            {showDashboard && dashboard && m ? (
                <div className="space-y-6" aria-busy={metricsBusy || undefined}>
                    ...
                    <section className="space-y-3">
                        ...
                        <div className="grid gap-3 sm:grid-cols-2">
                            ...
                            >
                                <LineBurnBars
                                    lines={
                                        (m.budgetLineItemBurn
                                            .value as BudgetLineBurn[]) ?? []
                                    }
                                />
                            </MetricCard>
                            <MetricCard
